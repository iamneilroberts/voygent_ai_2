				...
				try {
					...
					if (!instruction) {
						return {
							content: [{
								type: "text",
								text: `❌ No active instruction found with name: ${params.name}`
							}]
						};
					}
					...
					await this.logDatabaseError(env, {
						attempted_operation: "get_instruction",
						error_message: error.message || "Unknown error",
						table_names: "instruction_sets",
						suggested_tool: "get_instruction"
					});

					return {
						content: [{
							type: "text",
							text: `❌ Error retrieving instruction: ${error.message}`
						}]
					};
				}
			}
			...
			async (params) => {
				...
				try {
					...
					return {
						content: [{
							type: "text",
							text: `❌ Error listing instructions: ${error.message}`
						}]
					};
				}
			}
			...
			async (params) => {
				...
				try {
					...
					return {
						content: [{
							type: "text",
							text: `❌ Error searching instructions: ${error.message}`
						}]
					};
				}
			}
				...
				try {
					...
					if (result.meta.changes === 0) {
						return {
							content: [{
								type: "text",
								text: `❌ No instruction found with name: ${params.name}`
							}]
						};
					}
					...
					return {
						content: [{
							type: "text",
							text: `✅ Successfully updated instruction: ${params.name}`
						}]
					};
				} catch (error: any) {
					return {
						content: [{
							type: "text",
							text: `❌ Error updating instruction: ${error.message}`
						}]
					};
				}
			}
			...
			async (params) => {
				...
				try {
					...
					return {
						content: [{
							type: "text",
							text: `❌ Error retrieving preference: ${error.message}`
						}]
					};
				}
			}
			...
			async (params) => {
				...
				try {
					...
					if (params.preference_type === 'verbosity_level') {
						...
						if (!validLevels.includes(params.preference_value)) {
							return {
								content: [{
									type: "text",
									text: `❌ Invalid verbosity level. Must be one of: ${validLevels.join(', ')}`
								}]
							};
						}
					}
					...
					if (params.preference_type === 'verbosity_level') {
						...
						responseText = `📊 Verbosity Mode Updated!\n\nNew: ${icons[params.preference_value as keyof typeof icons]} **${params.preference_value.charAt(0).toUpperCase() + params.preference_value.slice(1)}** - ${descriptions[params.preference_value as keyof typeof descriptions]}\n\nYour preferences have been saved and will apply to all future sessions.`;
					}
					...
					return {
						content: [{
							type: "text",
							text: `❌ Error setting preference: ${error.message}`
						}]
					};
				}
			}
				...
				try {
					...
					for (const pref of preferences.results) {
						...
						if (pref.preference_type === 'verbosity_level') {
							...
							const icon = icons[pref.preference_value as keyof typeof icons] || '❓';
					...
					return {
						content: [{
							type: "text",
							text: `❌ Error listing preferences: ${error.message}`
						}]
					};
				}
			}
