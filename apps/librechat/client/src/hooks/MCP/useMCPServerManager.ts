...
export function useMCPServerManager({ conversationId }: { conversationId?: string | null }) {
  ...
  const autoStartupAppliedRef = useRef(false);

  useEffect(() => {
    if (autoStartupAppliedRef.current) {
      return;
    }
    ...
    const startupServers = configuredServers.filter((serverName) => {
      const serverConfig = startupConfig.mcpServers?.[serverName];
      return serverConfig?.startup !== false;
    });
    ...
    autoStartupAppliedRef.current = true;
    batchToggleServers(startupServers);
  }, [
    configuredServers,
    startupConfig?.mcpServers,
    batchToggleServers,
