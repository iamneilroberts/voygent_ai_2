...
export default function useEventHandlers({
  ...
  const setVoygentLastUsage = useSetRecoilState(store.voygentLastUsage);
  ...
  const finalHandler = useCallback(
    (data: TFinalResData, submission: EventSubmission) => {
      ...
      // Approximate token usage for status pill fallback (no worker required)
      try {
        const reqText = (requestMessage?.text || getAllContentText(requestMessage) || '').toString();
        const resText = (responseMessage?.text || getAllContentText(responseMessage) || '').toString();
        const approx = (t: string) => Math.max(0, Math.ceil(t.trim().length / 4));
        const inputTokens = approx(reqText);
        const outputTokens = approx(resText);
        const rawModel = (conversation?.model || submissionConvo?.model || '').toString();
        const m = rawModel.toLowerCase();

        // Approx pricing (USD per 1M tokens → per token).
        const perToken = (() => {
          // Anthropic Claude
          if (m.includes('haiku')) return { in: 0.8 / 1e6, out: 4 / 1e6, label: 'Haiku' };
          if (m.includes('sonnet')) return { in: 3 / 1e6, out: 15 / 1e6, label: 'Sonnet' };
          if (m.includes('opus')) return { in: 15 / 1e6, out: 75 / 1e6, label: 'Opus' };
          // OpenAI (approx; subject to change)
          if (m.includes('gpt-4o-mini') || m.includes('4o-mini'))
            return { in: 0.15 / 1e6, out: 0.6 / 1e6, label: 'GPT‑4o mini' };
          if (m.includes('gpt-4o-2024-05-13'))
            return { in: 5 / 1e6, out: 15 / 1e6, label: 'GPT‑4o (2024‑05‑13)' };
          if (m.includes('gpt-4o') || m.includes('4o'))
            return { in: 2.5 / 1e6, out: 10 / 1e6, label: 'GPT‑4o' };
          if (m.includes('o4-mini')) return { in: 1.1 / 1e6, out: 4.4 / 1e6, label: 'o4 mini' };
          if (m.includes('o4')) return { in: 2.5 / 1e6, out: 10 / 1e6, label: 'o4' };
          if (m.includes('o3-mini')) return { in: 1.1 / 1e6, out: 4.4 / 1e6, label: 'o3 mini' };
          if (m.includes('o3')) return { in: 2 / 1e6, out: 8 / 1e6, label: 'o3' };
          if (m.includes('gpt-4.1-nano') || m.includes('4.1-nano'))
            return { in: 0.1 / 1e6, out: 0.4 / 1e6, label: 'GPT‑4.1 nano' };
          if (m.includes('gpt-4.1-mini') || m.includes('4.1-mini'))
            return { in: 0.4 / 1e6, out: 1.6 / 1e6, label: 'GPT‑4.1 mini' };
          if (m.includes('gpt-4.1')) return { in: 2 / 1e6, out: 8 / 1e6, label: 'GPT‑4.1' };
          if (m.includes('gpt-4.5')) return { in: 75 / 1e6, out: 150 / 1e6, label: 'GPT‑4.5' };
          if (m.includes('gpt-4-1106') || m.includes('gpt-4-vision') || m.includes('turbo'))
            return { in: 10 / 1e6, out: 30 / 1e6, label: 'GPT‑4 Turbo' };
          if (m.includes('gpt-3.5')) return { in: 0.5 / 1e6, out: 1.5 / 1e6, label: 'GPT‑3.5' };
          // Default unknown
          return { in: 0, out: 0, label: rawModel || 'Model' };
        })();

        const price = inputTokens * perToken.in + outputTokens * perToken.out;
        setIsSubmitting(false);
        // Persist last usage in Recoil for the StatusBar to consume if API not configured
        setVoygentLastUsage({
          model: perToken.label,
          inputTokens,
          outputTokens,
          price,
          approximate: true,
