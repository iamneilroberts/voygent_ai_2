...
async function processRequiredActions(client, requiredActions) {
  ...
  const tools = requiredActions
    .map((action) => {
      ...
      // Routing override: ensure `continue_trip` uses prompt-instructions server
      // Some MCP servers expose the same tool name. When the model selects
      // `continue_trip_mcp_d1-database`, prefer the prompt-instructions variant instead.
      if (typeof toolName === 'string') {
        const mcpDelimiter = Constants.mcp_delimiter;
        if (
          toolName.startsWith(`continue_trip${mcpDelimiter}`) &&
          toolName.endsWith(`${mcpDelimiter}d1-database`)
        ) {
          toolName = `continue_trip${mcpDelimiter}prompt-instructions`;
        }
      }
  ...
  for (let i = 0; i < requiredActions.length; i++) {
    ...
    const handleToolOutput = async (output) => {
      // Special handling for render_trip_status: parse JSON, upload SVG, emit image artifact
      if (currentAction.tool === 'render_trip_status') {
        try {
          let parsed;
          try {
          ...
          const markdown = parsed?.markdown ?? (typeof output === 'string' ? output : '');
          const dataUrl = parsed?.image_data_url || '';

          // Prepare tool_call entry first with markdown-only output
          ...
          // Decode data URL (supports base64 or utf8)
          if (typeof dataUrl === 'string' && dataUrl.startsWith('data:image/svg+xml')) {
            let buffer;
            const base64Idx = dataUrl.indexOf(';base64,');
            const utf8Idx = dataUrl.indexOf(';utf8,');
            if (base64Idx !== -1) {
              const b64 = dataUrl.slice(base64Idx + ';base64,'.length);
              buffer = Buffer.from(b64, 'base64');
            } else {
              const idx = utf8Idx !== -1 ? utf8Idx + ';utf8,'.length : dataUrl.indexOf(',') + 1;
              const encoded = dataUrl.slice(idx);
              buffer = Buffer.from(decodeURIComponent(encoded), 'utf8');
            }

            // Persist as image file and emit IMAGE_FILE content (artifact)
          ...
          logger.error('render_trip_status artifact handling failed:', err);
          return {
            tool_call_id: currentAction.toolCallId,
...
async function loadAgentTools({ req, res, agent, signal, tool_resources, openAIApiKey }) {
  ...
  if (includesWebSearch) {
    const allowedDomains = appConfig.webSearch?.allowedDomains;
    const maxScrapes = appConfig.webSearch?.maxScrapes;
    webSearchCallbacks = createOnSearchResults(res, allowedDomains, maxScrapes);
  }
