 ...
 */
function createOnSearchResults(res, allowedDomains = [], maxScrapes = undefined) {
  const context = {
    sourceMap: new Map(),
    ...
    maxScrapes: typeof maxScrapes === 'number' ? maxScrapes : undefined,
    scrapeCount: 0,
  };
  ...
  function onSearchResults(results, runnableConfig) {
    ...
    // Optionally filter results by allowed domains
    if (context.allowedDomains.length > 0 && data) {
      const keep = (link) =>
        typeof link === 'string' &&
      ...
      if (Array.isArray(data.organic)) {
        data.organic = data.organic.filter((item) => keep(item?.link));
      }
      if (Array.isArray(data.topStories)) {
        data.topStories = data.topStories.filter((item) => keep(item?.link));
      }
    }
  ...
  function onGetHighlights(link) {
    ...
    if (context.allowedDomains.length > 0) {
      try {
        const u = new URL(link);
        const allowed = context.allowedDomains.some((d) => u.hostname.endsWith(d));
        if (!allowed) {
          return;
        }
      } catch {
    ...
    // Enforce max scrapes per query
    if (typeof context.maxScrapes === 'number') {
      if (context.scrapeCount >= context.maxScrapes) {
        return;
      }
      context.scrapeCount += 1;
    }
