...
const { wrapWithProgressTracking } = require('./trackMCPProgress');
...
function createToolInstance({ res, toolName, serverName, toolDefinition, provider: _provider }) {
  ...
  /**
   * Basic sanitization for D1 search-like inputs to avoid SQLite
   * "LIKE or GLOB pattern too complex" errors when upstream servers
   * construct wildcard-heavy queries from raw input.
   */
  const sanitizeD1Args = (args) => {
    if (!args || typeof args !== 'object') return args;
    const copy = { ...args };
    const simplify = (val) => {
      if (typeof val !== 'string') return val;
      // Collapse repeated wildcards and strip complex glob constructs
      let v = val.replace(/[\*%]{2,}/g, '*'); // collapse ** or %%
      v = v.replace(/[\[\]\?]/g, ''); // remove [, ], ? which can explode patterns
      v = v.replace(/\s+/g, ' ').trim();
      // Limit length to keep pattern manageable
      if (v.length > 120) v = v.slice(0, 120);
      return v;
    };
    for (const key of Object.keys(copy)) {
      if (/query|pattern|search/i.test(key)) {
        copy[key] = simplify(copy[key]);
      }
    }
    return copy;
  };
  ...
  const _call = async (toolArguments, config) => {
    ...
    try {
      ...
      const isD1 = serverName === 'd1-database';
      // Prefer trip_id over long identifier strings for D1 lookups
      let normalizedArgs = toolArguments;
      if (isD1 && normalizedArgs && typeof normalizedArgs === 'object') {
        const hasTripId = Object.prototype.hasOwnProperty.call(normalizedArgs, 'trip_id');
        if (hasTripId) {
          normalizedArgs = { ...normalizedArgs };
          delete normalizedArgs.trip_identifier;
          delete normalizedArgs.trip_name;
        }
      }
      const primaryArgs = isD1 && typeof normalizedArgs === 'object' ? sanitizeD1Args(normalizedArgs) : normalizedArgs;
      ...
      try {
        ...
        const patternErr = /LIKE or GLOB pattern too complex/i.test(msg);
        if (isD1 && patternErr && typeof toolArguments === 'object') {
          // Retry with aggressive sanitization: strip non-word chars and shorten
          const fallback = { ...toolArguments };
          for (const k of Object.keys(fallback)) {
            if (typeof fallback[k] === 'string') {
              let v = fallback[k]
  ...
  // Wrap _call with progress tracking for trip-related tools
  const wrappedCall = wrapWithProgressTracking(_call, toolName);

  const toolInstance = tool(wrappedCall, {
    schema,
