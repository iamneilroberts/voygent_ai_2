    ...
    if (tool === Tools.execute_code) {
      ...
      requestedTools[tool] = async () => {
        const allowedDomains = Array.isArray(webSearch?.allowedDomains)
          ? webSearch.allowedDomains.join(', ')
          : '';
        const maxScrapes = typeof webSearch?.maxScrapes === 'number' ? webSearch.maxScrapes : 3;
...
8. **Open at most ${maxScrapes} sources.** Prefer domains: ${allowedDomains || 'wikipedia.org'}.
9. **If a chosen page times out and is critical**, retry that single page once with a higher timeout; otherwise proceed with alternative sources.
`.trim();
